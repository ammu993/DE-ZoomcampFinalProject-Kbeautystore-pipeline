...
def write_bq(df, table_name: str):
    """
    Appends data to BigQuery partitioned by date.
    ...
    print(f"Written to BigQuery: {BQ_DATASET}.{table_name}")
...
def main():
    ...
    print(f"Spark Ingest - {target_date}")
    ...
    write_bq(stg_orders,      "stg_orders")
    write_bq(stg_order_items, "stg_order_items")
    write_bq(stg_products,    "stg_products")

    print(f"Ingest complete for {target_date}")
    spark.stop()
